...
int XmlDocument::openDocument(string fn)
{
  ...
  if (!document)
    throw string("ERROR WHILE OPENING XML DOCUMENT : " + fn);
  return true;
}
...
double XmlTag::getAttributeDouble(string AttributeName) const
{
  ...
  if (xmlTag)
    if (xmlHasProp(xmlTag, prop))
      {
	double ret;
	char *buff = (char *)xmlGetProp(xmlTag,
					prop);
	sscanf(buff, "%lf", &ret);
	free(buff);
	return ret;
...
ListXmlTag XmlTag::searchChildren(string childrenName) const
{
  ...
  while (position < childrenName.size())
    {
      string localString;
      while ((position < childrenName.size()) && (childrenName[position] != '/'))
	localString += childrenName[position++];
      position++;
      //cout << "LocalString --" << localString << "--" << endl;
      if (localString == "")
	{			// Chaine commence par un /
      ...
      else
	{
	  ...
	  for (unsigned int i = 0; i != lxt.size(); i++)
	      ...
	      else
		{
	      ...
	      for (unsigned int j = 0; j != lxt2.size(); j++)
		lxt3.insert(lxt3.end(), lxt2[j]);
	    }
	  lxt = lxt3;
	}
    }
