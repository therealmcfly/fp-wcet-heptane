...
namespace cfglib {
  ...
  /* Set program name */ void Program::SetName(string pgm_name) {
    name = pgm_name;
  }
   ...
   * entry point. */
  Cfg *Program::CreateNewCfg(t_address addr, ListOfString const &name) {
    Cfg *cfg = new Cfg(this, addr, name);
...
/* return the Cfg with a specified name, 0 if it does not exists */
  Cfg *Program::GetCfgByAddress(t_address vaddr) {
    ListOfString lnames;
    for (listOfCfg::iterator it(this->cfgs_list.begin()); it != this->cfgs_list.end(); ++it)
      {
	if ((*it)->GetAddress() == vaddr) { return *it ; }
      }
    return 0;
  }
  ...
  std::ostream & Program::WriteXml(std::ostream & os, Handle & hand)
  {
    ...
    os << "<PROGRAM " << "  id =\"" << hand.identify(this) << "\" " << "  name=\"" << this->name << "\" " << "  entry=\"" << hand.identify(this->entry_point) << "\" " << ">" << std::endl;
    int i = 0;
    for (listOfCfg::const_iterator it = this->cfgs_list.begin(); it != this->cfgs_list.end(); it++)
      {
  ...
  void Program::ReadXml(XmlTag const *tag, Handle & h) {
	...
	if (child.getName() == string("ATTRS_LIST"))
	  continue;
