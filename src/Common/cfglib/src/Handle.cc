...
namespace cfglib {
   ...
   * user. */ void Handle::addID_serialisable(std::string const &id, Serialisable * attr) {
  ...
  void Handle::resolveHandles() {
	...
	std::cout << "In resolveHandles, with ID = " << currentID << endl;
	...
	for (itset = currentToSolve.begin(); itset != currentToSolve.end(); itset++)
	  {
	    int n = id_serialisable.count(currentID);
	    assert(n != 0);
	    Serialisable **ptr = *itset;
