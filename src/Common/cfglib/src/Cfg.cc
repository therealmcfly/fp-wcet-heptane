...
namespace cfglib {
  ...
  Cfg::Cfg(Program * program, t_address addr, ListOfString name):program(program), addr(addr), name(name), external(true), startNode(0) 
  {
  }

  /*! constructor for a free Cfg (used in clone function) */ Cfg::Cfg(ListOfString name):name(name) {
  }
   ...
   * node is automatically set as the start node. */
  Node *Cfg::CreateNewNode(enum node_type type) {
    external = false;
  ...
  std::vector < Node * >Cfg::GetSuccessors(Node * node)
  {
	...
	if ((*it)->origin == node)
	  {
	    ndest = (*it)->destination;
	    succ.push_back(ndest);
	  }
      }
    return succ;
  }
  ...
  {
    /* os << "<CFG id=\"" << hand.identify((Serialisable *) this) << "\" " << " name=\"" << this->getStringName() << " \" cfgaddress=\"" << std::hex << this->GetAddress() << std::dec << "\" " << " startnode=\"" ;
    if (this->startNode != NULL) os << hand.identify((Serialisable *) this->startNode);
    os << "\" " << " endnodes=\"";
    */
    os << "<CFG id=\"" << hand.identify((Serialisable *) this) << "\" " << " name=\"" << this->getStringName() << " \" cfgaddress=\"" << 
      std::hex << this->GetAddress() << std::dec << "\" " << " startnode=\"" << hand.identify((Serialisable *) this->startNode) << "\" ";    
  ...
  void Cfg::ReadXml(XmlTag const *tag, Handle & hand) {
      ...
      if (children.size() != 0)
	{
	  if (attr_endnodes == "")
	    {
	      cout << "Cfg " << cfgname << " does not have endnodes" << endl;
	      cout << "Going on, ..." << endl;
	      //exit(-1);
	    }
