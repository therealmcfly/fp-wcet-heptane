...
namespace cfglib {
  ...
  {
    os << "<ATTR " << "type=\"integer\" " << "name=\"" << this->name << "\" " << "value=\"" << this->value << "\" " << "/>" << std::endl;
    return os;
  }
  ...
  {
    os << "<ATTR " << "type=\"float\" " << "name=\"" << this->name << "\" " << "value=\"" << this->value << "\" " << "/>" << std::endl;
    return os;
  }
  ...
  {
    os << "<ATTR " << "type=\"unsignedlong\" " << "name=\"" << this->name << "\" " << "value=\"" << this->value << "\" " << "/>" << std::endl;
    return os;
  }
  ...
  {
    os << "<ATTR " << "type=\"string\" " << "name=\"" << this->name << "\" " << "value=\"" << this->value << "\" " << "/>" << std::endl;
    return os;
  }
...
/*! Virtual constructor */
  ...
  SerialisableListAttribute *SerialisableListAttribute::clone(CloneHandle & handle) {
    ...
    for (element_it = this->value.begin(); element_it != this->value.end(); ++element_it)
      {
  ...
  {
    os << "<ATTR " << "type=\"list\" " << "name=\"" << this->name << "\" " << ">" << endl;
  ...
  /*! virtual constructor */
  SerialisableDoubleAttribute *SerialisableDoubleAttribute::clone() {
    return new SerialisableDoubleAttribute(*this);
  }

  SerialisableDoubleAttribute *SerialisableDoubleAttribute::create() {
    return new SerialisableDoubleAttribute();
  }

  /*! Attribute printing function (debug only) */
  void SerialisableDoubleAttribute::Print(std::ostream & s) {
    s << "(type=SerialisableDoubleAttribute,name=" << name << ",value=" << value << ")";
  }

  /*! Serialisation function */
  std::ostream & SerialisableDoubleAttribute::WriteXml(std::ostream & os, Handle & hand)
  {
    os << "<ATTR " << "type=\"double\" " << "name=\"" << this->name << "\" " << "value=\"" << this->value << "\" " << "/>" << std::endl;
    return os;
  }

  void SerialisableDoubleAttribute::ReadXml(XmlTag const *tag, cfglib::Handle & hand) {
    string attr_name = tag->getAttributeString(std::string("name"));
    assert(attr_name != "");
    this->name = attr_name;
    string val = tag->getAttributeString(std::string("value"));
