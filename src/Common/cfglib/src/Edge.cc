...
namespace cfglib {
  ...
  /*! basic constructor */
   Edge::Edge(Node * const origin, Node * const destination, Cfg * cfg):cfg(cfg), origin(origin), destination(destination) {
  }

  /*! uninitialised constructor */ Edge::Edge():cfg(0), origin(0), destination(0)
  {
  }
  ...
  std::ostream & Edge::WriteXml(std::ostream & os, Handle & hand)
  {
    ...
    if ( this->destination == NULL)
      {
	cout << " Edge " << hand.identify(this) << " The destination is NULL: perhaps the node " <<  hand.identify(this->origin) << " refers another node of another CFG " << endl; 
	dest="NULL";
      }
    else
      dest =  hand.identify(this->destination);

    os << "    <EDGE " << " id=\"" << hand.identify(this) << "\" ";
