...
namespace cfglib {
  ...
  Loop *Loop::Clone(CloneHandle & handle) {
    ...
    for (std::vector < Edge * >::iterator it = backedges.begin(); it != backedges.end(); it++)
      {
  ...
  {
    os << "<LOOP " << " id=\"" << hand.identify((Serialisable *) this) << "\" ";
    ...
    // List of backedges
    os << " backedges=\"";
    for (std::vector < Edge * >::const_iterator it(this->backedges.begin()); it != this->backedges.end(); ++it)
      {
	os << hand.identify((Serialisable *) * it);
	os << ", ";
      }
   ...
   * nested loops, call method GetAllNodesNotNested */ std::vector < Node * >Loop::GetAllNodes() const {
    return this->nodes;
  }
  /*! Get the nodes of the Loop. The vector's first Node is
   * the head of the loop, other nodes are in no particular
   ...
   */ std::vector < Node * >Loop::GetAllNodesNotNested() const {
	...
	bool is_in_nested_loop = false;
	  for (unsigned int l = 0; l < vl.size(); l++) {
	    if (vl[l] != this)
	      {
		if (vl[l]->FindInLoop(my_node) && vl[l]->IsNestedIn((Loop *) this))
		  {
		    is_in_nested_loop = true;
		    break;
	}}} if (!is_in_nested_loop)
	   res.push_back(my_node);
      }
    return res;
  }
   ...
   */ bool Loop::IsNestedIn(Loop * l) const {
    if (l == NULL)
      return true;		// NULL represents the cfg itself
  ...
  void Loop::Peel(void) {
	...
	if (node->IsCall())
	  {
	    copy->SetCall(node->GetCalleeName(), false);
	  }
	temp[node] = copy;

	// Can a node be a Cfg start node? If so, it is no longer in the
	// peeled loop.
	if (node == cfg->GetStartNode())
	  {
	    cfg->SetStartNode(copy);
	  }
    ...
    for (vector < Edge * >::iterator it_edges = all_edges.begin(); it_edges != all_edges.end(); it_edges++)
      {
	Edge *e = *it_edges;
    ...
    for (vector < Loop * >::iterator loop_it = loops.begin(); loop_it != loops.end(); ++loop_it)
      {
	// Find nested loops
	Loop *l = *loop_it;
	if (l == this)
	...
	for (vector < Node * >::iterator nested_it = nested_nodes.begin(); nested_it != nested_nodes.end(); ++nested_it)
	  {
	...
	for (vector < string >::const_iterator old_attr_it = old_attr.begin(); old_attr_it != old_attr.end(); ++old_attr_it)
	  {
	...
	for (vector < Edge * >::const_iterator back_edge_it = back_edges.begin(); back_edge_it != back_edges.end(); ++back_edge_it)
	  {
	    Edge *e = *back_edge_it;
    ...
    for (map < Node *, Node * >::iterator copy_it = temp.begin(); copy_it != temp.end(); ++copy_it)
      {
    ...
    if (HasAttribute("maxiter"))
      {
	SerialisableIntegerAttribute ia = (SerialisableIntegerAttribute &) GetAttribute("maxiter");
