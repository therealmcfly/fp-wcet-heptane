...
namespace cfglib {
  ...
  /*! constructor of a Node, common to BB and Code nodes. */ Node::Node(Cfg * cfg):cfg(cfg), type(BB), is_return(false), callee_cfg(NULL)
  {
    dbg_node(std::cout << "Node constructor called" << std::endl;
	);
  }
  /*! constructor. */ Node::Node(Cfg * cfg, node_type type):cfg(cfg), type(type), is_return(false), callee_cfg(NULL)
  {
    dbg_node(std::cout << "Node constructor called" << std::endl;
	);
  }
  ...
  /*! Returns the Cfg this Node belong to */ Cfg *Node::GetCfg()
  {
    return this->cfg;
  }
  ...
  node_type type_of_string(std::string stype) {
    if (std::string("BasicBlock") == stype)
      {
	return BB;
      }
    if (std::string("FunctionCall") == stype)
      {
	return Call;
      }
  ...
  {
    os << "  <NODE " << " id=\"" << hand.identify(this) << "\" " << " type=\"" << string_of_type(this->type) << "\" ";
    if (Call == this->type)
      {
  ...
  */
  bool Node::isIsolatedNopNode() {
    ...
    if (asm_code != "nop")
      return false;
    if (GetCfg()->GetPredecessors(this).size() != 0)
      return false;
