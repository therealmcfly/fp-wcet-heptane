...
#define ARCHI_HAS_GP_AREA() ((Arch::getArchitectureName() == "MIPS") || (Arch::getArchitectureName() == "RISCV") || (Arch::getArchitectureName() == "FLEXPRET"))
...
void SymbolTableAttribute::addSection(std::string name, unsigned long addr, int size)
{
	std::pair<unsigned long, int> tmp(addr, size);
	sections[name] = tmp;
}

void SymbolTableAttribute::addVariable(std::string name, unsigned long addr, int size, std::string section_name)
{
	triplet tmp(addr, size, section_name);
	variables[name] = tmp;
}

void SymbolTableAttribute::setGP(unsigned long val)
{
	_gp = val;
}
...
bool SymbolTableAttribute::getInfo(unsigned long addr, string *var_name, unsigned long *start_addr, int *size, string *section_name)
{
	// search in variables
...
ostream &SymbolTableAttribute::WriteXml(std::ostream &os, Handle &hand)
{
	...
	for (map<std::string, triplet>::const_iterator it = variables.begin(); it != variables.end(); it++)
	{
		triplet
				current = it->second;
		os << "<VARIABLE name=\"" << it->first << "\" addr=\"0x" << hex << current.first << dec << "\" size=\"" << current.second << "\" section=\"" << current.third << "\"/>" << std::endl;
	}
	os << "</VARIABLES>" << std::endl;

	//-- extra ----- Archi with GP ( MIPS, RISCV )
	if (ARCHI_HAS_GP_AREA())
	{
...
// Serialisation function
void SymbolTableAttribute::ReadXml(XmlTag const *tag, Handle &hand)
{
	...
	for (size_t c = 0; c < children.size(); c++)
	{
		...
		{
			if (ARCHI_HAS_GP_AREA() && child_name == "GP")
			{
...
// TODO
void SymbolTableAttribute::Print(std::ostream &os)
{
