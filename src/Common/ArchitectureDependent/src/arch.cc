...
// public constructor
void Arch::init(const string &arch, const bool is_big_endian, const string &dataPath)
{
	if (instance != NULL)
	{
		delete instance;
	...
	if (arch == "MIPS")
	{
		instance = new MIPS(is_big_endian, dataPath);
	}
	else if (arch == "ARM")
	{
		instance = new ARM(is_big_endian, dataPath);
	}
	else if (arch == "MSP430")
	{
		instance = new MSP430(is_big_endian, dataPath);
	}
	else if (arch == "RISCV")
	{
		instance = new RISCV(is_big_endian, dataPath);
	}
	else if (arch == "FLEXPRET")
	{
		instance = new FLEXPRET(is_big_endian, dataPath);
	}
	else
	{
...
bool Arch::isRepeat(const ObjdumpInstruction &instr)
{
	return getInstance()->isRepeat(instr);
}
...
string Arch::rebuiltObjdumpInstruction(const string &vcode, t_address addrinstr)
{
	return getInstance()->rebuiltObjdumpInstruction(vcode, addrinstr);
}
...
int Arch::getPipelineFlushCost()
{
	return getInstance()->getPipelineFlushCost();
}

int Arch::getBranchPredWrongPredCost(uint32_t memlatency, const string &code)
{
	return getInstance()->getBranchPredWrongPredCost(memlatency, code);
}
int Arch::getBranchPredCorrectPredGain(uint32_t memlatency, const string &code)
{
	return getInstance()->getBranchPredCorrectPredGain(memlatency, code);
}
int Arch::getBranchPredMissBTBCost(uint32_t memlatency, const string &code)
{
	return getInstance()->getBranchPredMissBTBCost(memlatency, code);
}
...
{
	if (regs.find(reg_name) != regs.end())
...
{
	/* Specificity of Indirect Mode in MSP430 */
	if (Arch::getArchitectureName() == "MSP430")
	{
		if (reg_name[0] == '@')
		{
			const string reg_name_bis = reg_name.substr(1, 3);
			return regs.find(reg_name_bis) != regs.end();
		}
	}
...
bool Arch_dep::isRepeat(const ObjdumpInstruction &instr)
{
	return getInstructionTypeFromMnemonic(instr.mnemonic)->isRepeat();
}
...
uint32_t Arch::getStackFrameReservedSize(const string &instr)
{
	return getInstance()->getStackFrameReservedSize(instr);
}
...
void Arch_dep::parseReadElfLine(const string &line, ObjdumpSymbolTable &table)
{
	...
	//  cout << " INFOS =" << number << ' ' << name << ' ' << type << ' ' << addr << ' ' << off << ' ' << size << endl;
	if (name != "NULL")
	{
		assert(number != "");
		assert(name != "");
		...
		if (find(sectionsToExtract.begin(), sectionsToExtract.end(), name) != sectionsToExtract.end())
		{
			...
			// LBesnard June 2019 assert(size_sect != 0 && size_sect != INT_MAX);
