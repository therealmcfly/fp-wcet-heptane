...
#define isUndefinedOperand(s) (s == "")
#define isDefinedOperand(s) (s != "")
...
void ARM_ADD::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  ...
  if (Arch::getInstr2ARMInfos(vinstr, codeinstr, oreg, &TypeOperand, operand1, operand2, operand3))
      ...
      else
	{
	  ...
	  string op = "vadd";	// vadd.XX
	  if (Arch::isARMClassInstr(codeinstr, op))
	    {
	      setRegistersInfos3ops(regs, precision, vstack, vStackPrecision);
	      if (Arch::isConditionnedARMInstr(codeinstr, op))	// possible ???
		killop1(regs, precision);
	      else
		add(regs, precision, precision[num_register1]);
	    }
...
void ARM_DIV::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  // add*, adc*
  DAA_TRACE("ARM_DIV", instructionAsm);
  string vinstr = instructionAsm;
  if (Arch::getInstr2ARMInfos(vinstr, codeinstr, oreg, &TypeOperand, operand1, operand2, operand3))
    {
      // Floating point operation.
      string op = "vdiv";	// vdiv.XX
      if (Arch::isARMClassInstr(codeinstr, op))
	{
	  setRegistersInfos3ops(regs, precision, vstack, vStackPrecision);
	  if (Arch::isConditionnedARMInstr(codeinstr, op))
...
void ARM_MOV::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  ...
  //  if (Arch::getInstr1ARMInfos(vinstr, codeinstr, oreg, &TypeOperand, operand1, operand2)) for the instruction "vmov Dn, Rd, Rn" changed by 
  if (Arch::getInstr2ARMInfos(vinstr, codeinstr, oreg, &TypeOperand, operand1, operand2, operand3))
    {
...
void ARM_LOAD::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  ...
  if (Arch::getLoadStoreARMInfos(true, vinstr, codeinstr, oreg, &vIndexAddressing, &TypeOperand, operand1, operand2, operand3))
	...
	{
	  if ( isUndefinedOperand(operand2) && isUndefinedOperand(operand3))
	    {
	      // ldr R1, [R2] and R2 = sp-val
	      int ireg = Arch::getRegisterNumber(operand1);
	      i_set = getStackIndexFromRegister(regs, ireg , &i);
	    }
	  else
	    i_set= false;
	}
      ...
      if (i_set)
	{
	  ...
	  int vindex;
	  if ( getEffectiveStackIndex(vstack, i, -1, &vindex))
	    {
	      string vstr = vstack[vindex];
	      regs[num_register0] = vstr;
	      precision[num_register0] = vStackPrecision[vindex];
	    }
	  else
	    {
	      cout << " for " << instructionAsm  << endl;
	      regs[num_register0] = "*";
...
void ARM_LOAD_MULTIPLE::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  ...
  if (Arch::getMultipleLoadStoreARMInfos(vinstr, codeinstr, oreg, regList, &writeBack))
      ...
      if (codeinstr == "ldm" || codeinstr == "ldmia")	// increment after.
	{
      ...
      else if (codeinstr == "ldmib")	// increment before.
	{
      ...
      else if (codeinstr == "ldmda")	// Decrement After
	{
      ...
      else if (codeinstr == "ldmdb")	// Decrement Before
	{
...
void ARM_STORE::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  ...
  if (Arch::getLoadStoreARMInfos(true, vinstr, codeinstr, oreg, &vIndexAddressing, &TypeOperand, operand1, operand2, operand3))
      ...
      if (! getStackIndex(operand1, operand2, operand3, &i))
	{
      ...
      if (i_set) // i<0 for a metainstruction (push, stmxx)
	{
	  num_register0 = Arch::getRegisterNumber(oreg);
	  int vindex;
	  if ( getEffectiveStackIndex(vstack, i, -1, &vindex))
	    {
	      string vstr = regs[num_register0];
	      vstack[vindex] = vstr;
...
void ARM_CONVERSION_DP_SP::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  string op;
  string vinstr = instructionAsm;
  if (Arch::getInstr1ARMInfos(vinstr, codeinstr, oreg, &TypeOperand, operand1, operand2))
    {
      setRegistersInfos2ops(regs, precision, vstack, vStackPrecision);
      op = "vcvt";		// vcvt.f64.f32, vcvt.f64.s32, vcvt.f64.u32, vcvt.s32.f64, vcvt.f32.f64, vcvt.f32.s32, vcvt.s32.f32. Format: rd_rs
      if (Arch::isARMClassInstr(codeinstr, op))
	{
	  if (Arch::isConditionnedARMInstr(codeinstr, op))
	    ERROR_ACCESS(" ARM_CONVERSION_DP_SP: NOT YET IMPLEMENTED, conditionned instruction ", instructionAsm);
	  else
	    {
	      if (TypeOperand == immediate_offset)
		loadConstant(regs, precision, regs[num_register1]);
	...
	ERROR_ACCESS("ARM_CONVERSION_DP_SP_BAD_OPERATOR", instructionAsm);
    }
  else
    ERROR_ACCESS("ARM_CONVERSION_DP_SP ", instructionAsm);
}

void ARM_NEGATE_FP::simulate(regTable & regs, regPrecisionTable & precision, stackType & vstack, stackPrecType & vStackPrecision, const string & instructionAsm)
{
  string op;
  string vinstr = instructionAsm;
  DAA_TRACE("ARM_TODO_LOIC, ARM_NEGATE_FP not effect", instructionAsm);
  return; // TODO -- CAUTION Floating-point negate operator.
  

  if (Arch::getInstr1ARMInfos(vinstr, codeinstr, oreg, &TypeOperand, operand1, operand2))
    {
      setRegistersInfos2ops(regs, precision, vstack, vStackPrecision);
      op = "vneg";		// vcvt.f64.f32, vcvt.f64.s32, vcvt.f64.u32, vcvt.s32.f64, vcvt.f32.f64, vcvt.f32.s32, vcvt.s32.f32. Format: rd_rs
      if (Arch::isARMClassInstr(codeinstr, op))
	{
	  if (Arch::isConditionnedARMInstr(codeinstr, op))
	    ERROR_ACCESS(" ARM_NEGATE_FP: NOT YET IMPLEMENTED, conditionned instruction ", instructionAsm);
	  else
	    {
	      if (TypeOperand == immediate_offset)
		loadConstant(regs, precision, regs[num_register1]);
	...
	ERROR_ACCESS("ARM_NEGATE_FP_BAD_OPERATOR", instructionAsm);

    }
  else
    ERROR_ACCESS("ARM_NEGATE_FP_DP_SP ", instructionAsm);
}
