...
private:
   ...
   ~Utl ();
public:
   /** @return the definition of a shell variable (varName) or "UNDEFINED" if it is not set.*/
   static string getEnvDef(string varName);

   /** @return "true" if the HEPTANE_FORCE_MODE shell variable is set to TRUE or true or 1, "false"otherwise*/
   static bool getForceMode( );
   ...
   /** @return "true" if the operand string str is a integer value in decimal, "false" otherwise.*/
   static bool isDecNumber(const string& str);

   /** @return "true" if the operand string str is a real value in decimal, "false" otherwise.*/
   static bool isFPNumber(const string& str);
   ...
   static string int2string(long v);
   ...
   /** @return "true" if the expression (operand1 % operand2) is evaluated to v, "false" otherwise. */
   static bool evalRemainder(string &operand1, string &operand2, long *v);

   /** @return "true" if the expression (operand1 / operand2) is evaluated to v, "false" otherwise. */
   static bool evalDivide(string &operand1, string &operand2, long *v);

   static bool evalNegate(string &operand1, long *v);
   ...
   static bool getValueAfter (string & Addrl, size_t ifrom, long *val);

   /** @return a demangle string  */
   static string demangle(const string &); // Benjamin
   /** @return true if a file exists*/
   static bool file_exists(const string &filename); // Benjamin
   static long hexa2dec(const string& hexavalue);
   static bool parseRefStackPointer(const string &vstr, int *i);
   static bool getNextChar(const string &vstr, size_t *vindex );

   static int scanIntegerfrom(string vtr, int ifrom); // for MSP430 clean instruction
   static void removeAndKill(string & vstr, int ifrom, char c);// for MSP430 clean instruction
