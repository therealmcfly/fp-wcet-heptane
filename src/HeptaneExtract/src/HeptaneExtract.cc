...
/** *****************************************************
    ...
    Only MIPS and ARM, MSP430 and RISC-V architectures are currently supported
...
using namespace std;
static bool isARMArchi = false, isMIPSArchi = false, isMSP430Archi = false, isRISCVArchi = false;
static bool opt_verbose = false;
static ObjdumpSymbolTable symbol_table;


static void GenEntryPoints( string exportDir ) {
 typeTableFunctions::const_iterator it;
 std::ofstream output_file_stream;
 string exportfilename = exportDir + "/" + "user_functions.txt";
 output_file_stream.open(exportfilename.c_str());

 for (it = symbol_table.userFunctions.begin(); it != symbol_table.userFunctions.end(); it++)
    {
      output_file_stream << *(it->second.begin()) << " ";
    }
 output_file_stream << endl;
}

/**
 * Display some statistics about the program
 * As for now:
 *      - number of functions
 *      - min/avg/max number of BB per program
 *      - min/avg/max number of instruction per BB
 *      - min/avg/max number of conditional branch per program
 *      - min/avg/max number of memory instruction per program
 * @param cfglib_program
 */
static void DisplayStats(cfglib::Program & cfglib_program)
{
  uint32_t nbBBs = 0;
  uint32_t nbInstrs = 0;
  vector < uint32_t > sizeBBs;
  uint32_t nbCondBranch = 0;
  uint32_t nbMemInstr = 0;

  cout << "----------------------------------------------" << endl;
  cout << "\t\tStatistics" << endl;
  cout << endl;

  for (Cfg * cfg:cfglib_program.GetAllCfgs())
    {
      nbBBs += cfg->GetAllNodes().size();
      uint32_t lnbinstr = 0;
      uint32_t lstacksize = 0;
      for (Node * node:cfg->GetAllNodes())
	{
	  if (!node->GetInstructions().size())
	    continue;

	  sizeBBs.push_back(node->GetInstructions().size());
	  nbInstrs += node->GetInstructions().size();
	  lnbinstr += node->GetInstructions().size();
	  for (Instruction * instr:node->GetInstructions())
	    {
	      if (UtlCfgLib::isCondJump(instr))
		nbCondBranch++;
	      else if (UtlCfgLib::isLoad(instr) || UtlCfgLib::isStore(instr))
		nbMemInstr++;
	      lstacksize += Arch::getStackFrameReservedSize(instr->GetCode());
	    }
	}
      //cout << "-> " << cfg->getStringName() << " -- <memfootprint local-storage=\"" << lstacksize << "\" data-type=\"bit\" code-size=\"" << lnbinstr*Arch::getInstructionSize()*8 << " b\" />" << endl;
      SerialisableIntegerAttribute acs(lnbinstr * Arch::getInstructionSize() * 8);
      cfg->SetAttribute("CodeSize", acs);
      SerialisableIntegerAttribute ass(lstacksize);
      cfg->SetAttribute("StackSize", ass);
      /*
	cout << "-> " << cfg->getStringName() << endl;
	cout << "\tNB BBs : " <<  cfg->GetAllNodes().size() << endl;
	cout << "\tCode size : " << lnbinstr*Arch::getInstructionSize()*8 << " bits" << endl;
	cout << "\tStack size : " << lstacksize << " bits" << endl;
	cout << endl;
	cout << "\t\t\t----------" << endl;
      */
    }

  auto result = std::minmax_element(sizeBBs.begin(), sizeBBs.end());
  uint32_t acc = 0;
  for (uint32_t a:sizeBBs)
    acc += a;

  cout << endl;
  //cout << "----------------------------------------------" << endl;
  //cout << endl;
  cout << "Number of functions: " << cfglib_program.GetAllCfgs().size() << endl;
  cout << "NB BBs : " << nbBBs << endl;
  if (sizeBBs.size() > 0)
    cout << "Nb Instrs/BBs: " << *result.first << ' ' << *result.second << " -- " << (acc / sizeBBs.size()) << '\n';
  cout << "Nb Instrs : " << nbInstrs << endl;
  cout << "Nb cond branch: " << nbCondBranch << endl;
  cout << "Nb mem instr: " << nbMemInstr << endl;
  cout << "----------------------------------------------" << endl;
  cout << endl;
}
...
*/
static void exportCfg(cfglib::Program & cfglib_program, string exportDir)
{
  std::ofstream output_file_stream;
  string exportXMLfilename = exportDir + "/" + cfglib_program.GetName() + ".xml";
...
bool isMSP430EpilogFunction(t_address addr, string * nbpop)
{
  string epilog = "__mspabi_func_epilog_";
  typeTableFunctions::const_iterator it;
  for (it = symbol_table.functions.begin(); it != symbol_table.functions.end(); it++)
    {
      if (it->first == addr)
	{
	  ListOfString lnames = it->second;
	  string name = *(lnames.begin());
	  //__mspabi_func_epilog_INTEGER
	  size_t index = name.find(epilog);
	  if (index == EOS)
	    return false;
	  if (index != 0)
	    return false;
	  *nbpop = name.substr(epilog.length(), name.length());
	  // cout << " Function found , its name is " << name << " its address is " << std::hex << addr << std::dec << " nbpop " << *nbpop << endl;
	  return true;
	}
    }
  return false;
...
bool changeCodeMSP430(ObjdumpInstruction & vinstruction)
{
  string nbpop;

  if (!isMSP430Archi)
    return false;
  if (!Arch::isUnconditionalJump(vinstruction))
    return false;
  // if unconditonnal branching to an epilog, the code is modified. (MSP430)
  //  string text_instruction = vinstruction.asm_code;
  // if this address is the address of a CFG; and it is an epilog, the instruction is modified.
  t_address addr = Arch::getJumpDestination(vinstruction);
  if (isMSP430EpilogFunction(addr, &nbpop))
    {
      // cout << " changeCodeMSP430, Epilog for new instruction " << vinstruction.asm_code << endl; 
      vinstruction.asm_code = "ret " + nbpop;
      return true;
    }
  return false;
}
		  ...
		  vector < ObjdumpWord > >&instrWithWords, map < int, ObjdumpInstruction > &MetaInstructionsTable)
{
  ...
  // cout << " Creating CFG,  name = " << function.name << " address = " << std::hex << function.addr <<  std::dec << endl;
  // cfglib::Cfg * cfgcur = cfglib_program.GetCfgByName(function.name);
  cfglib::Cfg * cfgcur = cfglib_program.GetCfgByAddress(function.addr);
  if (cfgcur == (cfglib::Cfg *) 0)
    {
      // removed for MSP430, but returns false.
  ...
  for (size_t i = 0; i < instructions.size(); i++)
      ...
      if (prev_addr != current_addr)
	{
      ...
      bool bret;
      if (changeCodeMSP430(current_instruction))
	bret = true;
      else
	bret = Arch::isReturn(current_instruction);

      string text_instruction = current_instruction.asm_code;
      cfglib::Instruction * inst = current_node->CreateNewInstruction(text_instruction, cfglib::Code, bret);
      ...
      if (Arch::isCall(current_instruction))
	{
	  ...
	  if (Arch::getCalleeName(current_instruction) == "")
	    Logger::addFatal("CFG extractor: name of called empty returned by getCalleeName \n\t(probably an indirect call or a switch), instruction: " + current_instruction.asm_code);
	      ...
	      for (set < t_address >::const_iterator it = current_successors.begin(); it != current_successors.end(); it++)
		{
		  if (!changeCodeMSP430(current_instruction))
		    { 
		      ...
		      // ADDED LBESNARD 23 DECEMBER 2019. MSP430: a node of a CFG may refer to a node of another cfg.
		      /*  if (nsuc == NULL)
			  {
			  if (isMSP430Archi )
			  {
			  std::cout << "MSP430: the successor of a node is NULL (A node of a CFG perhaps refers to a node of another CFG) ... exiting" << endl;
			  exit(-1);
			  }
			  assert(false);
			  } */
...
void verifsOnCfg(vector < cfglib::Cfg * >lcfg) {
  for (unsigned int c = 0; c < lcfg.size(); c++) {
    Cfg *acfg = lcfg[c];
    std::vector < Loop * > loops = acfg->GetAllLoops();
    for (std::vector < Loop * >::iterator it = loops.begin(); it != loops.end(); it++)
      {
	if ( (*it) -> GetAllNodes().size() == 1)
	  cout << " Warning: Loop with one basic bloc in CFG: " <<  acfg->getStringName() << endl;
      }
...
/**
   ...
   - Manage annotations.
*/
static void finalize_program_construction(const ConfigExtract & config, cfglib::Program & cfglib_program)
{
  // Make sure functions are correctly linked for call nodes
  ...
  if (entry_cfg == NULL)
    Logger::addFatal("CFG extractor: can't set entry point to undefined Cfg " + config.entry_point_name);
  ...
  for (unsigned int c = 0; c < lcfg.size(); c++)
	...
	{
	  if (bswitch)
	    SwitchComputer::computeSwitch(acfg, lswitches, isMIPSArchi || isMSP430Archi || isRISCVArchi);
  ...
  if (config.binary_only)
    AttachAnnotationsFromXML(cfglib_program, config.annotation_file, opt_verbose);
  ...
  if (config.output_annot)
    GenerateAnnotationXMLFile(cfglib_program, config.result_dir + "/" + config.annotation_file, opt_verbose);
  verifsOnCfg(lcfg);
}
...
{
  string bsens, vcodeInstr, asm_code, codeinstr, oregister;
  ...
  if (Arch::getMultipleLoadStoreARMInfos(asm_code, codeinstr, oregister, regList, &writeBack))
    {
      ...
      if (isaPush)
	{ 
	  // For push, to be conform with the stack managing.
	  instr_copy = instr;
	  v = 4*regList.size();
	  instr_copy.asm_code =  "sub " + oregister + ", " + oregister + ", " + int2string(v);
	  instructions.push_back(instr_copy);
	  MetaInstructionsTable[0] = instr;
	  v = -4;
	  nbadded++;
	  for (size_t i = 0; i < regList.size(); i++)
	    {
	      instr_copy = instr;
	      v = v + 4;
	      instr_copy.asm_code = vcodeInstr + regList[i] + ", [" + oregister + ((v == 0) ? "" : ", " + int2string(v)) + "]";
	...
	{ 
	  bool bIncr = (codeinstr.find("i") != EOS);
	  bsens = (bIncr ? "" : "-");
	  bool isAfter = (codeinstr.find("a") != EOS);
	  v = 0;
	  for (size_t i = 0; i < regList.size(); i++)
	    {
	      instr_copy = instr;
...
*/
static void BuildCfg(const ConfigExtract & config)
{
  string objdumpname = config.result_dir + '/' + config.program_name + ".objdump";
  ifstream input(objdumpname.c_str(), ios::in);
  if (!input.is_open())
    Logger::addFatal("Error: file " + objdumpname + " not present");
  string readelfname = config.result_dir + '/' + config.program_name + ".readelf";
  ifstream input_readelf(readelfname.c_str(), ios::in);
  if (!input_readelf.is_open())
    Logger::addFatal("Error: file " + readelfname + " not present");
  ...
  isMSP430Archi = Arch::getArchitectureName() == "MSP430";
  isRISCVArchi  = Arch::getArchitectureName() == "RISCV";
  ...
  while (!input_readelf.eof())
    {
      getline(input_readelf, line);
      if (Arch::isReadelfFlagsMarker(line))
	break;			// If the line is "Key to Flags:"
  ...
  while (!input.eof() && !Arch::isObjdumpTextMarker(line))
    {
      getline(input, line);
      if (line.length() == 0 || Arch::isObjdumpTextMarker(line))
	continue;
      //      cout << " *** parsing the line "  << line << endl;
  ...
  for (it = symbol_table.functions.begin(); it != symbol_table.functions.end(); it++)
    {
      cfglib_program.CreateNewCfg(it->first, it->second);
      // cout << "Creation du cfg avec it->first = " << std::hex << it->first << std::dec << " second =" << *(it->second.begin()) <<  endl;
    }
  ...
  // specific MIPS, RISCV: set GP
  if (isMIPSArchi || isRISCVArchi) ts_attribute.setGP(symbol_table.gp);
  ...
  while (!input.eof())
	  ...
	  if (function.name != "")
	    {
	      ...
	      if (build_heptane_cfg(cfglib_program, bb_start_addr, instructions, function, succs, instrWithWords, MetaInstructionsTable))
		{
      ...
      else if (Arch::isInstruction(line))	// instruction
	{
	  ...
	  if (isARMArchi && Arch::isWord(instr))
	    continue;
	  ...
	  if (Arch::isCall(instr))
	    ...
	    {
	      t_address addr_next_bb;
	      if (isMSP430Archi)
		{
		  addr_next_bb = instr.addr + 2 + Arch::getNBInstrInDelaySlot() * Arch::getInstructionSize();
		}
	      else
		{
		  addr_next_bb = instr.addr + Arch::getInstructionSize() + Arch::getNBInstrInDelaySlot() * Arch::getInstructionSize();
		}
	      bb_start_addr.insert(addr_next_bb);
  ...
  exportCfg(cfglib_program, config.result_dir);

  if (config.display_stats)
    DisplayStats(cfglib_program);

  GenEntryPoints(config.result_dir);
}
