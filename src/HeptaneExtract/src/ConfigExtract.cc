...
void ConfigExtract::CompileAll(ListXmlTag & lt)
{
  ...
  if (binary_file == false)
      ...
      for (unsigned int i = 0; i < lt.size(); i++)
	{
	  ...
	  if (file_type == "C")
		...
		Logger::addFatal("ConfigExtract error: PREPROCESSOR should be specified");
	      string cfile = tmp_dir + "/" + basename + ".c";
	      string ccpcommand = pre_processor + " " + pre_processor_args + " " + file_name + ">" + cfile;
	      execute_cmd(ccpcommand);
	      if ( pre_processor_script != "")
		{
		  string cppscriptcmd =  pre_processor_script + " "  + cfile;
		  execute_cmd(cppscriptcmd);
		}
		...
		Logger::addFatal("ConfigExtract error: COMPILER should be specified");
	      ...
	      if ( compiler_script != "")
		{
		  string asmscriptcmd =  compiler_script + " "  + asmfile;
		  execute_cmd(asmscriptcmd);
		}
	...
	linkcmd += " -T " + linker_script;
      // cout << "LINK COMMAND= " << linkcmd << endl;
      execute_cmd(linkcmd);
  ...
  // Apply readelf and generate the output file
  if (readelf == "") 
    Logger::addFatal("ConfigExtract error: READELF should be specified");
  string readelfcmd = readelf + " " + readelf_args + " -S " + prefix_tmpfile + ">" + prefix_tmpfile + ".readelf";
  execute_cmd(readelfcmd);
  if(output_readelf)
    copy( prefix_tmpfile + ".readelf ", result_dir + "/" + program_name + ".readelf ");
  ...
  if (objdump_script != "")
    {
      string scriptobjdumpcmd;
      scriptobjdumpcmd = objdump_script + " "  + prefix_tmpfile + ".objdump";
      execute_cmd(scriptobjdumpcmd);
    }
...
ConfigExtract::ConfigExtract(string filename, bool verbose)
{
  ...
  if (lt.size() == 1)
    {
      ...
      pre_processor_script = lt[0].getAttributeString("SCRIPT");
    }
  ...
  if (lt.size() == 1)
    {
      ...
      compiler_script = lt[0].getAttributeString("SCRIPT");
    }
  ...
  if (lt.size() == 1)
    {
      ...
      objdump_script = lt[0].getAttributeString("SCRIPT");
    }
  ...
  // cfg statistics (from Benjamin Rouxel)
  lt = xmldoc.searchChildren("STATISTICS");
  assert(lt.size() <= 1);
  if (lt.size() == 1)
    {
      string vstat = lt[0].getAttributeString("VALUE");
      display_stats = (vstat == "YES");
    }
  // -----------------------
      ...
      for (int i = 0; i < n; i++)
	{
	  ...
	  if (file_type != "BINARY" && file_type != "C" && file_type != "ASM" && file_type != "")
		...
		if (annotation_file == "")
		  ...
		  {
		    ifstream an;
		    string annotfile = string(input_dir+"/"+annotation_file);
		    an.open(annotfile.c_str());
		    if (an.fail())
		      {
			Logger::addFatal("ConfigExtract error: ANNOTATIONFILE not found: " + annotfile);
		      }
		    binary_only = true;
		  }
	    }
