...
#include "MSP430AddressAnalysis.h"
#include "Utl.h"

// #define TRACE(S) S

/**
   MSP430: This is used to get the size of the stack frame of a function with its first asm instruction
   return the size of the stack frame. 
*/
int MSP430AddressAnalysis::getStackSize (Cfg * cfg)
{
  // Get the first instruction to get the stack size  (MSP430 example : addiu sp, sp, -24 )
  ...
  // a leaf in the call graph (no dedicated stack frame in MSP430) is accepted.
  assert ( ! isCfgWithCallNode(cfg));
  return 0;
}

/* mem_pattern is assume to be "val(sp)"
void MSP430AddressAnalysis::extractRegVal( string mem_pattern, string &reg, string &val)
{
...
int MSP430AddressAnalysis::getStackMaxOffset(string s_instr, int StackMaxOffset)
{
  string val, reg;
...
void MSP430AddressAnalysis::analyzeStack (Cfg * cfg, Instruction * Instr, long offset, string access, int sizeOfMemoryAccess, bool precision, Context *context)
{
  ...
  if (stack_maxoffset < offset)
	...
	{
	  addr = sp;
	  // TODO: check the MSP430 ABI if the parameters of the caller can be accessed if not stack_size_caller can be used instead
...
void MSP430AddressAnalysis::analyzeReg (Instruction * Instr, long addr, string access, int sizeOfMemoryAccess, bool precision)
{
  string var_name;
...
long  MSP430AddressAnalysis::GetOffsetValue(string asm_code)
{
...
bool MSP430AddressAnalysis::setLoadStoreAddressAttribute(Cfg * vCfg, Instruction* vinstr, RegState *state, Context *context)
{
  ...
  // lui is not a load. --See MSP430.cc : mnemonicToInstructionTypes["lui"]    =new Basic("alu",formats["rd_hex"],new Lui())--; 
  // Load MSP430= { ["lw"], ["lwl"], ["lwr"], ["lwc1"], ["lh"], ["lhu"], ["lb"], ["lbu"], ["ldc1"] }
  // Store MSP430 = { ["sw"], ["swc1"], ["sh"], ["sb"], ["sdc1"] }
...
RegState* MSP430AddressAnalysis::NewRegState(int stackSize)
{
  return new MSP430RegState(MSP430_NB_REGISTERS, stackSize);
}

int MSP430AddressAnalysis::getNbWordsForStack(Cfg * c)
{
  ...
  return  attribute.getFrameSizeWithCaller (); // attribute.getFrameSizeWithoutCaller ();
}



bool MSP430AddressAnalysis::importCallerArguments(AbstractRegMem &vAbstractRegMemCaller, AbstractRegMem &vAbstractRegMemCalled)
{
...
void MSP430AddressAnalysis::printPointerAccessInfos(Instruction* vinstr)
{ 
  string vmess;
  vmess = "  POINTER ACCESS in setPointerAccessInfo( " + vinstr->GetCode() + ')';
  cout << vmess ;
  long vaddr;
  if (getCodeAddress(vinstr, &vaddr))
...
MSP430AddressAnalysis::MSP430AddressAnalysis (Program * p, int sp):AddressAnalysis (p,sp)
{ }

bool
MSP430AddressAnalysis::PerformAnalysis ()
{
...
bool
MSP430AddressAnalysis::CheckInputAttributes ()
{
...
//nothig to remove
void
MSP430AddressAnalysis::RemovePrivateAttributes ()
{
