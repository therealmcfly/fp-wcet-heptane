...
bool MIPSAddressAnalysis::TryToComputeStackSize( const vector < Instruction * > &listInstr, int ifrom, int iTo, int *stackSize)
{
  // it seems that it does not occur in MIPS.
  return false; 
}
...
int MIPSAddressAnalysis::getStackSize (Cfg * cfg)
{
  ...
  if (v_instr.size () == 4 && v_instr[1] == "sp" && v_instr[2] == "sp")
    {
      // addiu sp, sp, -24 
      int stackSize = Utl::string2int(v_instr[3]); 
      if (stackSize < 0) stackSize = -stackSize;
      return stackSize;
      // Assumed : no modification of the stack after.
    }

  int vindex;
  // Verifying that there is no sp modification in the other instructions.
  if ( isStackModified (listInstr, 1, &vindex))
    {
      if ( TryToComputeStackSize(listInstr, 0, vindex, &stackSize)) return stackSize;
      Logger::addError("The stack pointer is modified in the middle of a node");
      return -1;
    }
...
bool MIPSAddressAnalysis::analyzeStack (Cfg * cfg, Instruction * Instr, long offset, string access, int sizeOfMemoryAccess, bool precision, Context *context)
{
...
bool MIPSAddressAnalysis::setLoadStoreAddressAttribute(Cfg * vCfg, Instruction* vinstr, RegState *state, Context *context)
{
  ...
  TRACE_setLoadStoreAddressAttribute( 
          cout << " Begin setLoadStoreAddressAttribute  instruction = " << asm_code << endl;
	  state->printStack();  state->printStates();  
  );
  ...
  TRACE_setLoadStoreAddressAttribute( 
          cout << " END setLoadStoreAddressAttribute  instruction = " << asm_code << endl;
	  state->printStack();  state->printStates();  
  );
...
{
  return new MIPSRegState(MIPS_NB_REGISTERS, stackSize);
}
...
void MIPSAddressAnalysis::printPointerAccessInfos(Instruction* vinstr)
{ 
  string vmess;
