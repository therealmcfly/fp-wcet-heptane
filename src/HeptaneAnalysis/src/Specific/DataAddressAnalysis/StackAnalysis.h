...
class StackAnalysis: public Analysis
{
 ...
 private:
  ...
  int reqStackSize;  ///< required size for the stack.
  bool VerifModifStackExcludingFirstAndEndNodes (Cfg * cfg);
...
protected:
  bool stackAnalysis (); ///< Stack analysis starting point
    bool isStackModified( const vector < Instruction * > &listInstr, int ifrom, int *vindex);

public:
  StackAnalysis (Program * p, int sp);
  int GetRequiredStackSize();
};
