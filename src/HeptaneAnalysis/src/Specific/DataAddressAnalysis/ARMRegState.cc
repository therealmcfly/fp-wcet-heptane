...
ARMRegState::ARMRegState(int nbregs, int stackSize):RegState(nbregs, stackSize)
{
  setPredefValues();
}
...
void ARMRegState::setPredefValues()
{
  ...
  state[ARM_LR_REGISTER] = "lr"; // Link  Register, (lr) for test.
  precision[ARM_LR_REGISTER] = true;
}
...
int ARMRegState::getSPRegister()
{
  return ARM_SP_REGISTER;
}
...
void ARMRegState::simulate(Instruction * vinstr)
{
  ...
  assert(precision[ARM_SP_REGISTER]);
  ...
  if (!b)
    {
      ...
      if (!b)
	cout << " LBesnard: ARMRegState::getValueSP :: not evaluated expression = " << oper << endl;
      return b;
    }
  *val = atol(oper.c_str());
  return true;
...
bool ARMRegState::isAccessAnalysisSP(int register_number, string offset, vector < string > &result)
{
  ...
  if (getValueSP(value, &val))
    ...
    {
      // changed LBesnard September 2019.
...
vector < string > ARMRegState::makeAnalysisWordInfos(string vtype, unsigned long val, unsigned long addr)
{
  ...
  if (bimmediate)
    result.push_back(strAddr);

  TRACE(if (vtype == "imm") 
	  cout << " loading the value " << strAddr << endl;
	else
	  cout << " loading the variable at adress 0x" << std::hex << val << std::dec << " ( long = " << val << " )" << endl) ;
  return result;
}
...
bool ARMRegState::isAccessAnalysisOtherRegister(Instruction * instr, string codeinstr, int register_number, string offset, offsetType TypeOperand, vector < string > &result)
{
  ...
  if (found != EOS) 
    {
	 ...
	 cout << " ARMRegState::isAccessAnalysisOtherRegister for " <<  instr->GetCode() <<  " , value = " << value << " at " <<  std::hex <<  addrInstruction  <<  std::dec << endl;
      */
      return false;
    }
  ...
  /*   for (int i=0; i < result.size(); i++)    {      cout << "    result[" << i << "] = " << result[i] << endl;    } */
  return true;
}
...
bool ARMRegState::GetWordPCrelative(Instruction * instr, offsetType TypeOperand, string offset, string codeinstr, string * vtype, unsigned long *val, unsigned long *addr)
{
  ...
  if (Arch::isConditionnedARMInstr(codeinstr, "vldr"))
    return false;
...
bool ARMRegState::GetWordAt(Instruction * instr, string regvalue, string offset, offsetType TypeOperand, string codeinstr, string * vtype, unsigned long *val, unsigned long *addr)
{
  ...
  if (Arch::isConditionnedARMInstr(codeinstr, "vldr")) return false;
...
void ARMRegState::printInstrInfos(string & instr, string & codeinstr, string & oregister, bool & pre_indexed_addr, offsetType & TypeOperand, bool & WriteBackNewAddress, string & operand1,
				  string & operand2, string & operand3)
{
  ...
  if (WriteBackNewAddress)
    cout << "---- Write Back New Address " << endl;
  cout << "---- Operand 1 = " << operand1 << endl;
  cout << "---- Operand 2 = " << operand2 << endl;
  cout << "---- Operand 3 = " << operand3 << endl;
}

string ARMRegState::getAliasRegister(int ireg)
{
  string reg = Utl::int2string(ireg);
  if (ireg == 13) return "sp";
  if (ireg == 14) return "lr";
  if (ireg == 15) return "pc";
  return 'r'+reg;
}
