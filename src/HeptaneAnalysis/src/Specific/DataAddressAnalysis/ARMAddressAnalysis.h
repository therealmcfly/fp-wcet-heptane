...
class ARMAddressAnalysis:public AddressAnalysis
{
 private:
  ...
  bool analyzeStack (Cfg * cfg, Instruction * Instr, long offset, string access, int sizeOfMemoryAccess, bool precision, Context *context);
  void analyzeReg (Instruction * Instr, long addr, string access, int sizeOfMemoryAccess, bool precision, RegState * state);
  int getStackSize(Instruction * vinstr);
 protected:
  int getStackSize (Cfg * cfg);
  int getStackSize(const vector < Instruction * >&listInstr, int *ifrom);
  ...
  void printPointerAccessInfos(Instruction* vinstr);
  void printInstrMessage(string prefix, Instruction* vinstr);
