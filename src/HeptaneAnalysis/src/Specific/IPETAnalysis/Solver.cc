...
void Solver::setFrequencyAttribute(string VariableName, string freq)
{
  ...
  if (n != NULL)
      ...
      if (n->HasAttribute(attr))
	{
      ...
      n->SetAttribute(attr, frequency); // attribute name is FrequencyAttributeName + _ + context
    }
}
...
// -------------------------------------------------------------
void LpsolveSolver::generate_objective_function(ostringstream & os, vector < string > ids, vector < long >cst)
{
  os << "MAX: " << endl;
...
// - ids: ids of variables in the linear system
// - cst: constant in the linear system
void LpsolveSolver::generate_linear_inequality(ostringstream & os, vector < string > ids, vector < long >cst, int N)
{
  assert(ids.size() == cst.size());
  unsigned int nvars = ids.size();
  for (unsigned int i = 0; i < nvars; i++)
...
/**
   It declares types of variables as integers (needed by lp_solve)
*/
void LpsolveSolver::generate_declarations(ostringstream & os, vector < string > ids)
{
  string avar;
  for (unsigned int i = 0; i < ids.size(); i++)
    {
      avar = ids[i];
      if (find(binvars.begin(), binvars.end(), avar) != binvars.end()) os << "bin ";
      os << "int " << avar << ";" << endl;
    }
}

void  LpsolveSolver::generate_null_variable(ostringstream & os, string varid) {
  os <<  varid << " = 0;" << endl;
}

// Generate a flow constraint
// vid[0] = sum(vid[1..n])
void LpsolveSolver::generate_flow_constraint(ostringstream & os, vector < string > vid)
{
  assert(vid.size() > 0);
  if (vid.size() > 1)
    {
...
// Sum(vids) <= N
void LpsolveSolver::generate_inequality(ostringstream & os, vector < string > vid, int N)
{
  assert(vid.size() > 0);
...
// Generate an inequality
// Sum(vids) = N
void LpsolveSolver::generate_equality(ostringstream & os, vector < string > vid, int N)
{
  assert(vid.size() > 0);
  unsigned int nvar = vid.size();
  for (unsigned int i = 0; i < nvar; i++)
...
void LpsolveSolver::generate_int2bin(ostringstream & os, const string & X, const string & q)
{
  assert(false && "LPSolve doesn't really work well, so I gave up maintaining it");
}

bool LpsolveSolver::solve(string file_name, string fout)
{
...
bool LpsolveSolver::parse_output(string file_name, string & wcet)
{
  ...
  while (!fin.fail())
	...
	{
	  /* wcet_found = true;
	     std::size_t  dotfound = s5.find(".");
	     if (dotfound != string::npos) s5.erase(dotfound);
	     wcet = s5; */

	  double fwcet = atof(s5.c_str());
	  char buf[256];
...
// - cst: constant in the linear system
// -------------------------------------------------------------
void CPLEXSolver::generate_objective_function(ostringstream & os, vector < string > ids, vector < long >cst)
{
  assert(ids.size() == cst.size());
...
// - ids: ids of variables in the linear system
// - cst: constant in the linear system
void CPLEXSolver::generate_linear_inequality(ostringstream & os, vector < string > ids, vector < long >cst, int N)
{
  assert(ids.size() == cst.size());
  unsigned int nvars = ids.size();
  for (unsigned int i = 0; i < nvars; i++)
...
void CPLEXSolver::generate_declarations(ostringstream & os, vector < string > ids)
{
  os << "General " << endl;
  for (string s:ids)
    {
      if (find(binvars.begin(), binvars.end(), s) == binvars.end())
	os << s << endl;
    }
...
void  CPLEXSolver::generate_null_variable(ostringstream & os, string varid) {
  os <<  varid << " = 0" << endl;
}

// Generate a flow constraint
// vid[0] = sum(vid[1..n])
void CPLEXSolver::generate_flow_constraint(ostringstream & os, vector < string > vid)
{
  assert(vid.size() > 0);
  if (vid.size() > 1)
    {
...
// Generate an inequality
// Sum(vids) <= N
void CPLEXSolver::generate_inequality(ostringstream & os, vector < string > vid, int N)
{
  assert(vid.size() > 0);
  unsigned int nvar = vid.size();
  for (unsigned int i = 0; i < nvar; i++)
...
// Generate an inequality
// Sum(vids) = N
void CPLEXSolver::generate_equality(ostringstream & os, vector < string > vid, int N)
{
  assert(vid.size() > 0);
  unsigned int nvar = vid.size();
  for (unsigned int i = 0; i < nvar; i++)
...
void CPLEXSolver::generate_int2bin(ostringstream & os, const string & X, const string & q)
{
  os << q << " = 0 -> " << X << " = 0" << endl;
  binvars.insert(q);
}
...
bool CPLEXSolver::solve(string file_name, string fout)
{
  ofstream file;
  ...
  // cout << " calling cplex with " <<  lp_solve_command << endl;
  if (system(lp_solve_command.c_str()) != 0)
    {
      stringstream errorstr;
...
bool CPLEXSolver::parse_output(string file_name, string & wcet)
{
  ...
  while (!readfile.eof() && (!found_wcet || !found_SolutionStatus))
      ...
      if (line.find("solutionStatusString") != EOS)
	{
      ...
      else if (line.find("objectiveValue") != EOS)
	{
