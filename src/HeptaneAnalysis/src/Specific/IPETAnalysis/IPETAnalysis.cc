...
void IPETAnalysis::printBasicBlockInfos(Node *n, string vtext)
{
...
int IPETAnalysis::getIPETMethodToApply(bool pipeline, bool perfectIcache, bool perfectDcache)
{
	if (pipeline)
	{
		std::cout << "[IPET DEBUG] Pipeline method selected" << std::endl;
		std::cout << "[IPET DEBUG] perfectIcache: " << perfectIcache << std::endl;
		std::cout << "[IPET DEBUG] perfectDcache: " << perfectDcache << std::endl;
		...
		{
			// FLEXPRET-specific: Allow pipeline with perfect icache for scratchpad memory simulation
			string archName = Arch::getArchitectureName();
			if (archName == "FLEXPRET")
			{
				Logger::print("*** IPET analysis: FLEXPRET pipeline with perfect instruction cache (scratchpad memory).");
				if (!perfectDcache)
					return METHOD_PIPELINE_ICACHE_DCACHE;
				return METHOD_PIPELINE_ICACHE_PERFECTDCACHE;
			}
...
// ---------------------------------------
IPETAnalysis::IPETAnalysis(Program *p, int used_solver, bool pipeline, bool generate_wcet_info, bool generate_node_freq, int nb_icache_levels,
...
/**  */
bool IPETAnalysis::isReachableState(Context *c)
{
	if (isNULLPointer(c))
		return true;
		...
		if (!isDeadCode(vCFGCallee))
		{
				...
				for (unsigned int ic = 0; ic < nc; ic++)
				{
					...
					if (!isNULLPointer(n))
					{
						if (isReachableState(callee_context))
						{
							...
							string s1 = mkVariableNameSolver("n_", n, caller_context->getStringId());
							ncallers.push_back(s1);
							string s2 = mkVariableNameSolver("n_", start, callee_context->getStringId());
			...
			else
			{
				...
				for (unsigned int ic = 0; ic < nc; ic++)
				{
					...
					Context *caller_context = callee_context->getCallerContext();
					vector<string> ncallers;
					string s = mkVariableNameSolver("n_", n, caller_context->getStringId());
...
unsigned int IPETAnalysis::DataCacheLevel_NextLevel(string classif, unsigned int memBlock, unsigned int frequency, bool *never_accessed_data, bool *always_accessed_data,
...
void IPETAnalysis::generateConstraints_NOPIPELINE_CACHE(vector<Node *> vn, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
	string contextName;
	unsigned int i, ic, nc;
...
void IPETAnalysis::generateConstraints_NOPIPELINE_NOCACHE(vector<Node *> vn, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
	Node *n;
	int wcet;
	string contextName;
	...
	for (i = 0; i < vn.size(); i++)
	{
		...
		for (ic = 0; ic < nc; ic++)
		{
			...
			string s = mkVariableNameSolver("n_", n, contextName);
			vid.push_back(s);
...
void IPETAnalysis::generateConstraints_PIPELINE_CACHE(Cfg *c, vector<Node *> vn, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
	...
	for (unsigned int i = 0; i < vn.size(); i++)
	{
		...
		for (unsigned int ic = 0; ic < nc; ic++)
		{
			...
			// Incoming edges
			{
				...
				vector<string> vs;
				string snode1 = mkVariableNameSolver("n_", n, contextName);
				vs.push_back(snode1);
			...
			// Outgoing edges
			{
				...
				vector<string> vs;
				string snode2 = mkVariableNameSolver("n_", n, contextName);
				vs.push_back(snode2);
 ...
 */
bool IPETAnalysis::BBLoopMustBeConstrained(Cfg *c, Node *node, Node *head, Loop *loop)
{
...
void IPETAnalysis::generateConstraints_PIPELINE_ICACHE_DCACHE(ostringstream &os, Cfg *c, vector<Node *> &vn, vector<Edge *> &ve, const ContextList &contexts, vector<string> &vid,
																															VECTOR_WCET &vwcet)
{
...
void IPETAnalysis::generateConstraints_PIPELINE_ICACHE_PERFECTDCACHE(ostringstream &os, Cfg *c, vector<Node *> &vn, vector<Edge *> &ve, const ContextList &contexts, vector<string> &vid,
																																		 VECTOR_WCET &vwcet)
{
...
void IPETAnalysis::generateConstraints_NOPIPELINE_ICACHE_DCACHE(ostringstream &os, Cfg *c, vector<Node *> &vn, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
...
void IPETAnalysis::generateConstraints_NOPIPELINE_ICACHE_PERFECTDCACHE(ostringstream &os, Cfg *c, vector<Node *> &vn, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
...
void IPETAnalysis::generateConstraints_NOPIPELINE_PERFECTICACHE_DCACHE(ostringstream &os, Cfg *c, vector<Node *> &vn, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
...
void IPETAnalysis::generateConstraints_NOPIPELINE_PERFECTICACHE_PERFECTDCACHE(ostringstream &os, Cfg *c, vector<Node *> &vn, const ContextList &contexts, vector<string> &vid,
																																							VECTOR_WCET &vwcet)
{
...
*/
void IPETAnalysis::generateConstraints_IPET_selected_method(ostringstream &os, Cfg *c, vector<Node *> &vn, vector<Edge *> &ve, const ContextList &contexts, vector<string> &vid,
																														VECTOR_WCET &vwcet)
{
	switch (method)
	{
...
void IPETAnalysis::generateConstraints_ExternalFunction(Cfg *c, const ContextList &contexts, vector<string> &vid, VECTOR_WCET &vwcet)
{
	if (c->HasAttribute(ExternalWCETAttributeName))
	{
...
bool IPETAnalysis::generateConstraints(ostringstream &os, Cfg *c, vector<string> &vid, VECTOR_WCET &vwcet)
{
	vector<Node *> vn;
	vector<Edge *> ve = c->GetAllEdges();
...
bool IPETAnalysis::isDeadCode(Cfg *cfg)
{
...
bool IPETAnalysis::PerformAnalysis()
{
	...
	Cfg *function = config->getEntryPoint();
	...
	{
		Node *start_node = function->GetStartNode();
