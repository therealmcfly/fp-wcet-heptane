...
{
  if (ACCEPT_NO_CODELINE_ATTRIBUTE) return true;
  vector < Instruction * >vi = n->GetAsm ();
  for (unsigned int i = 0; i < vi.size (); i++)
    {
      if (! vi[i]->HasAttribute (CodeLineAttributeName)) return false;
    }
  return true;
}
...
HtmlPrint::CheckInputAttributes ()
{
  ...
  for (unsigned int i = 0; i < lcfg.size (); i++)
	...
	{
	  if ( ! AnalysisHelper::applyToAllCfgNodes (vcfg, CheckInstrHaveCodeLine, NULL))
	    {
	  ...
	  if (! cg->isDeadCode (lcfg[i]))
	    {
	      if (! AnalysisHelper::applyToAllCfgNodes (vcfg, CheckNodeHaveFrequency, NULL))
		{
		  stringstream errorstr;
  ...
  for (unsigned int i = 0; i < fileList.size (); i++)
      ...
      else
	{
	  if ( ! ACCEPT_NO_CODELINE_ATTRIBUTE) 
	    ...
	    {
	      cout  << "HtmlPrint: problem opening file:" << fileList[i] << endl;
	    }
	}
    }
...
LineFrequency::associateLineFreq (Cfg * cfg)
{
  ...
  for (unsigned int i = 0; i < nodes.size (); i++)
      ...
      for (unsigned int j = 0; j < insts.size (); j++)
	{
	  ...
	  if (curInst->HasAttribute (CodeLineAttributeName)) // Added LBesnard MSP430 (the objdump code may be modified) ACCEPT_NO_CODELINE_ATTRIBUTE 
	    {
