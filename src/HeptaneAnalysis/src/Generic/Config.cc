...
#include "Specific/PipelineAnalysis/FLEXPRETPipelineAnalysis.h"
...
//
// ---------------------------------------------------
void Config::FillArchitectureFromXml(string xml_file)
{
	...
	for (unsigned int i = 0; i < ltarch.size(); i++)
	{
		...
		if (currName == "TARGET")
		{
			...
			if ((arch_name != "MIPS") && (arch_name != "ARM") && (arch_name != "MSP430") && (arch_name != "RISCV") &&
					(arch_name != "FLEXPRET")) // Added FLEXPRET
			{
		...
		else if (currName == "CACHE")
		{
			...
			else if ((cp->type == DCACHE) && perfectDcache)
			{
			...
			else
			{
				...
				if ((tCache == ICACHE || tCache == PERFECTICACHE) && cp->level > nb_icache_levels)
					nb_icache_levels = cp->level;
				if ((tCache == DCACHE || tCache == PERFECTDCACHE) && cp->level > nb_dcache_levels)
					nb_dcache_levels = cp->level;
				if (tCache == PERFECTICACHE)
				{
					if (perfectIcache)
						Logger::addFatal("Config: configuration file should have atmost one perfect instruction cache directive");
					if (cp->level != 1)
					{
				...
				else if (tCache == PERFECTDCACHE)
				{
					if (perfectDcache)
						Logger::addFatal("Config: configuration file should have atmost one perfect data cache directive");
					if (cp->level != 1)
					{
						Logger::addWarning("Config: the level for a  perfect instruction cache must be 1, set to 1");
						cp->level = 1;
...
ParamAnalysis *Config::getParameters(string directive, string input_output_dir, XmlTag analysis)
{
	...
	{
		return new ParamEntryPoint(analysis);
	}

	// Printers ::= DOTPRINT | SIMPLEPRINT | HTMLPRINT | CODELINE | CACHESTATISTICS
	if (directive == "DOTPRINT")
	{
		return new ParamDotPrint(input_output_dir, analysis); /* Read paramaters from Xml */
	}
	if (directive == "SIMPLEPRINT")
	{
		return new ParamSimplePrint(analysis);
	}
	if (directive == "DUMMYANALYSIS")
	{
		return new ParamDummyAnalysis(analysis);
	}
	if (directive == "SESEANALYSIS")
	{
		return new ParamSESEAnalysis(analysis);
	}
	if (directive == "CODELINE")
	{
		return new ParamCodeLine(analysis);
	}
	if (directive == "HTMLPRINT")
	{
		return new ParamHtmlPrint(analysis);
	}
	if (directive == "CACHESTATISTICS")
	{
		return new ParamCacheStatistics(analysis);
	}

	// Analysis ::= ICACHE | PIPELINE | IPET | DATAADDRESS | DCACHE | DUMMYANALYSIS
	if (directive == "ICACHE")
	{
		return new ParamICache(analysis);
	}
	if (directive == "DATAADDRESS")
	{
		return new ParamDataAddress(analysis);
	}
	if (directive == "DCACHE")
	{
		return new ParamDCache(analysis);
	}
	if (directive == "PIPELINE")
	{
		return new ParamPipeline(analysis);
	}
	if (directive == "IPET")
	{
		return new ParamIPET(analysis);
	}
...
int Config::getPerfectIcacheLatency()
{
	if (perfectIcache)
	{
		if (cache_params[1][0]->type == PERFECTICACHE)
			return cache_params[1][0]->latency;
		if (cache_params[1][1]->type == PERFECTICACHE)
			return cache_params[1][1]->latency;
	}
	return -1;
}
...
int Config::getPerfectDcacheLatency()
{
	if (perfectDcache)
	{
		if (cache_params[1][0]->type == PERFECTDCACHE)
			return cache_params[1][0]->latency;
		if (cache_params[1][1]->type == PERFECTDCACHE)
			return cache_params[1][1]->latency;
	}
	return -1;
}
...
Analysis *Config::mkAnalyzerObject(string directive, Program *p, ParamAnalysis *pa)
{
	...
	if (directive == "DOTPRINT")
	{
		return new DotPrint(p, ((ParamDotPrint *)pa)->directory, ((ParamDotPrint *)pa)->detailed);
	}
	if (directive == "SIMPLEPRINT")
	{
	...
	if (directive == "DUMMYANALYSIS")
	{ /* ParamDummyAnalysis *ps=(ParamDummyAnalysis*)pa; */
		return new DummyAnalysis(p);
	}
	if (directive == "SESEANALYSIS")
	{
		return new SESEAnalysis(p);
	}

	if (directive == "CODELINE")
	{
		return new CodeLine(p, input_output_dir + "/" + ((ParamCodeLine *)pa)->binary_file, arch_name, ((ParamCodeLine *)pa)->binary_addr2line);
	}
	if (directive == "HTMLPRINT")
	{
		return new HtmlPrint(p, input_output_dir + "/" + ((ParamHtmlPrint *)pa)->html_file, ((ParamHtmlPrint *)pa)->colorize);
	}
	if (directive == "CACHESTATISTICS")
	{
	...
	if (directive == "ICACHE")
	{
		...
		if (ps->level > MaxLevelCacheAnalysis)
			MaxLevelCacheAnalysis = ps->level;
	...
	if (directive == "DATAADDRESS")
	{
		...
		if (arch_name == "MIPS")
			return new MIPSAddressAnalysis(p, ps->sp);
		if (arch_name == "ARM")
			return new ARMAddressAnalysis(p, ps->sp);
		if (arch_name == "MSP430")
			return new MSP430AddressAnalysis(p, ps->sp);
		if (arch_name == "RISCV")
			return new RISCVAddressAnalysis(p, ps->sp);
		if (arch_name == "FLEXPRET")
			return new RISCVAddressAnalysis(p, ps->sp); // Added for FLEXPRET which is RISCV-based
	...
	if (directive == "PIPELINE")
	{
		...
		if (arch_name == "MIPS")
			return new MIPSPipelineAnalysis(p, nbicache);
		if (arch_name == "ARM")
			return new ARMPipelineAnalysis(p, nbicache);
		if (arch_name == "MSP430")
			return new MSP430PipelineAnalysis(p, nbicache);
		if (arch_name == "RISCV")
			return new RISCVPipelineAnalysis(p, nbicache);
		if (arch_name == "FLEXPRET")
			return new FLEXPRETPipelineAnalysis(p, nbicache); // Added for FLEXPRET which is RISCV-based
...
void Config::ExecuteFromXml(string xml_file, bool printTime)
{
	XmlDocument xmldoc(xml_file);
	ListXmlTag lt;
	bool b;
	...
	if (lt.size() != 1)
	{
		Logger::addFatal("Config: there should be a single ANALYSIS tag in your XML");
	}
	...
	for (unsigned int i = 0; i < ltanalysis.size(); i++)
	{
		...
		if (analysis_name == "ENTRYPOINT")
		{
			...
			if (!p->SetEntryPoint(ep))
				Logger::addFatal("Config: Bad entry point name " + ep);
			b = true;
		}
		if (b)
		{
		...
		if (analysis_name != "ENTRYPOINT")
		{
			std::cout << "\n[CONFIG DEBUG] Analysis: " << analysis_name << std::endl;
			...
			if (!res)
				Logger::addFatal("Config: call to analysis failed");
			Logger::print();
...
CacheParam::CacheParam(XmlTag const &tag)
{
	...
	{
		if (stype == "picache")
			this->type = PERFECTICACHE;
		else
			this->type = PERFECTDCACHE;
		this->latency = tag.getAttributeInt("latency");
		assert(this->latency >= 0);
		this->level = tag.getAttributeInt("level");
		assert(this->level == 1);
...
// ------------------------------
ParamDotPrint::ParamDotPrint(string dir, XmlTag const &tag) : ParamAnalysis(tag)
{
	directory = dir;
...
ParamSimplePrint::ParamSimplePrint(XmlTag const &tag) : ParamAnalysis(tag)
{
...
// -----------------------------------------------------------------------------
ParamCodeLine::ParamCodeLine(XmlTag const &tag) : ParamAnalysis(tag)
{
	this->binary_file = tag.getAttributeString("binaryfile");
	this->binary_addr2line = tag.getAttributeString("addr2lineCommand");
}

ParamHtmlPrint::ParamHtmlPrint(XmlTag const &tag) : ParamAnalysis(tag)
{
...
// -----------------------------------------
ParamICache::ParamICache(XmlTag const &tag) : ParamAnalysis(tag)
{
	this->level = tag.getAttributeInt("level");
...
ParamDCache::ParamDCache(XmlTag const &tag) : ParamAnalysis(tag)
{
	this->level = tag.getAttributeInt("level");
	assert(level > 0 && level <= NB_MAX_CACHE_LEVEL);
...
// -----------------------
ParamDataAddress::ParamDataAddress(XmlTag const &tag) : ParamAnalysis(tag)
{
...
// Pipeline analysis
// -----------------
ParamPipeline::ParamPipeline(XmlTag const &tag) : ParamAnalysis(tag)
{
}

// WCET calculation
// ----------------
ParamIPET::ParamIPET(XmlTag const &tag) : ParamAnalysis(tag)
{
	string s;
...
ParamEntryPoint::ParamEntryPoint(XmlTag const &tag) : ParamAnalysis(tag)
{
	entrypoint = tag.getAttributeString("entrypointname");
	if (entrypoint == "")
		Logger::addFatal("Config: entry point name not set");
}

// Cache statistics functions (code/data)
// --------------------------------------
ParamCacheStatistics::ParamCacheStatistics(XmlTag const &tag) : ParamAnalysis(tag)
{
}

// DummyAnalysis
// --------------------------------------
ParamDummyAnalysis::ParamDummyAnalysis(XmlTag const &tag) : ParamAnalysis(tag)
{
}

// SESEAnalysis
// --------------------------------------
ParamSESEAnalysis::ParamSESEAnalysis(XmlTag const &tag) : ParamAnalysis(tag)
{
}
