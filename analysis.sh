...
then
    cd ${RESULT_DIR}
    sed  -e "s#BENCH_DIR#${RESULT_DIR}#g" -e "s/X_BENCH/${BENCH}/g" -e "s/_ENTRY_POINT_/${ENTRYPOINT}/g"  -e "s/_SOLVER_/${SOLVER}/g"  -e "s#_CROSS_COMPILER_DIR_#${HERE}/CROSS_COMPILERS/${XARCH}/bin#g" ${HERE}/config_files/configWCET_template_${XARCH}.xml > ${CONFIGFILE}
    chmod gou+x  ${CONFIGFILE}
    ${HERE}/bin/HeptaneAnalysis ${OPTION}  ${CONFIGFILE}  | tee ${RESULT_DIR}/analysis_${BENCH}_${XARCH}_${SOLVER}.log
