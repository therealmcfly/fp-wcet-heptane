...
Machine::Run()
{
  // Allocate storage for instruction decoding
  Instruction *instr = new Instruction;
  bool nextstop;//use to execute the last nop of the main function
  nextstop=false;
  ...
  for (;;) {
      ...
      //printf("\n gp register content : %d\n",int_registers[28]);//DH
	  if(nextstop)return;
      if (int_registers[NextPCReg]==0x4008) nextstop=true;/*return;*/
  }
...
{
  int vint[2];
  ASSERT(reg+1 < NumFPRegs);
#ifdef HOST_IS_BIG_ENDIAN
  vint[1] = machine->float_registers[reg];
  vint[0] = machine->float_registers[reg+1];
#else
  vint[0] = machine->float_registers[reg];
  vint[1] = machine->float_registers[reg+1];
#endif
  return *((double*) vint);
}
...
static void set_double(int reg,double val)
{
  ...
  vint = (int *) (&val);
#ifdef HOST_IS_BIG_ENDIAN
  machine->float_registers[reg] = vint[1];
  machine->float_registers[reg+1] = vint[0];
#else
  machine->float_registers[reg] = vint[0];
  machine->float_registers[reg+1] = vint[1];
#endif
}
...
{
  int raw;                      // binary representation of the instruction
  int nextLoadReg = 0; 	
  int nextLoadValue = 0; 	// record delayed load operation, to apply
				// in the future
  int execution_time,fetch_time;// execution time of the instruction
  ...
  if (!mmu->ReadMem(int_registers[PCReg], 4, &raw, true))
  ...
  if (DebugIsEnabled('m')) {
    ...
    printf("At PC = 0x%x: ", int_registers[PCReg]);
  ...
  int32_t pcAfter = int_registers[NextPCReg] + 4;
  int32_t sum, diff, value;
  uint32_t rs, rt, imm;
  ...
  case OP_ADD:
	...
	break;
      ...
      case OP_BEQ:
	if (int_registers[(int)instr->rs] == int_registers[(int)instr->rt])
	    pcAfter = int_registers[NextPCReg] + IndexToAddr(instr->extra);
	break;
	
      case OP_BGEZAL:
	int_registers[R31] = int_registers[NextPCReg] + 4;
      case OP_BGEZ:
	...
	if (int_registers[(int)instr->rt] == 0) {
	    int_registers[LoReg] = 0;
	    int_registers[HiReg] = 0;
	} else {
	    int_registers[LoReg] =  int_registers[(int)instr->rs]
	      / int_registers[(int)instr->rt];
	    int_registers[HiReg] = int_registers[(int)instr->rs]
	      % int_registers[(int)instr->rt];
	}
	break;
	  ...
	  if (rt == 0) {
	      ...
	      tmp = rs / rt;
	      int_registers[LoReg] = (int) tmp;
	      tmp = rs % rt;
	      int_registers[HiReg] = (int) tmp;
	  }
	  break;
	...
	break;
      ...
      case OP_JALR:
	int_registers[(int)instr->rd] = int_registers[NextPCReg] + 4;
      case OP_JR:
	...
	if (!mmu->ReadMem(tmp, 4, &value,false))
	  return 0;
	if (int_registers[LoadReg] == (int)instr->rt)
	    nextLoadValue = int_registers[LoadValueReg];
	else
	...
	break;
      ...
      case OP_MFHI:
	int_registers[(int)instr->rd] = int_registers[HiReg];
	break;
	
      case OP_MFLO:
	int_registers[(int)instr->rd] = int_registers[LoReg];
	break;
	...
	Mult(int_registers[(int)instr->rs], int_registers[(int)instr->rt], true,
	     &int_registers[HiReg], &int_registers[LoReg]);
	break;
	...
	break;
      ...
      case OP_SYSCALL:
	printf("Syscalls not supported ... exiting\n");
	exit(-1);
    ...
    case OP_ABS_D:
      d1 = get_double((int)instr->fs);
      set_double((int)instr->fd,fabs(d1));
      break; 
    ...
    case OP_ADD_D: {
      d1 = get_double((int)instr->fs);
      d2 = get_double((int)instr->ft);     
      set_double((int)instr->fd,d1+d2);
    }
      break; 
    ...
    case OP_NEG_D:
      d1 = get_double((int)instr->fs);
      set_double((int)instr->fd,-1.0*d1);
      break; 
    ...
    case OP_SQRT_S:
      ...
      fsptr = (float *) &(float_registers[(int)instr->fs]);
      if (*fsptr <0) {
	RaiseException(OverflowException,0); return 0;
      }
	...
	RaiseException(OverflowException,0); return 0;
      ...
      set_double((int)instr->fd,sqrt(d1));
      break;
   ...
   case OP_CVT_D_S:
	...
	(*((float*) &float_registers[(int)instr->fs]));
      break;
    case OP_CVT_D_W:
      set_double((int)instr->fd,(double)float_registers[(int)instr->fs]);
      break;
    case OP_CVT_W_D:
      float_registers[(int)instr->fd] = (int) get_double((int)instr->fs);
      break;
    ...
    // Signal it and stop Nachos
    case OP_RES:
	RaiseException(IllegalInstrException, int_registers[PCReg]);
	return 0;
    ...
    int_registers[PrevPCReg] = int_registers[PCReg];

    // Advance program counters.
    int_registers[PCReg] = int_registers[NextPCReg];
    int_registers[NextPCReg] = pcAfter;
...
{
    int_registers[int_registers[LoadReg]] = int_registers[LoadValueReg];
    int_registers[LoadReg] = nextReg;
    int_registers[LoadValueReg] = nextValue;
