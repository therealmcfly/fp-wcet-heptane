...
void initbuffer(void)
{
   ...
   for (i = 0 ; i < BUFFERSIZE ; i++) {
     ANNOT_MAXITER(50); // Read code, BUFFERSIZE == 50. - BL
...
void compress(void)
{
   ...
   {
	     ANNOT_MAXITER(8);  // Read code, depends on HSIZE == 257. - BL
      hshift++;
   }
   ...
   {
	     ANNOT_MAXITER(49); // Read code, InCnt size depends on IN_COUNT (50). - BL
...
probe:
  ANNOT_MAXITER(49);  // Read code, max value of in_count is 49 (iterations of the enclosing loop.). - BL
...
void cl_hash(count_int hsize)		/* reset code table */
{
   ...
   do {				/* might use Sys V memset(3) here */
	  ANNOT_MAXITER(16);  // Read code, HSIZE == 257. - BL
...
void writebytes( char *buf, int n )
{
   ...
   for( i=0; (i<n) && /*apsim*/ (i < BITS) ; i++ ) {
     ANNOT_MAXITER(16);	 // Read code, BITS == 16. - BL
   ...
   if ( code >= 0 ) {
      ...
      *bp = (*bp & rmask[r_off]) | (code << r_off) & lmask[r_off];  /* apsim_unknown buf */
      ...
      if ( offset == (n_bits << 3) ) {
         ...
         bytes_out += bits;
         do {
	   ANNOT_MAXITER(16);	 // Read code, BITS == 16, bp is initialised to buf, and bp - buf < BITS. - BL
            putbyte(*bp++);
