...
uint_least8_t EXTERNAL GetFreeRecord(void)
...

{
   ...
   if (telemetry_pointer < telemetry_end_pointer)
   {
      ...
      signed int index = (unsigned char *)telemetry_pointer - (unsigned char *)&telemetry_data;
      //where = &(((unsigned char *)&telemetry_data)[index]);
      tm_byte = ((unsigned char *)&telemetry_data)[index];
      // *telemetry_pointer;
      ...
      tm_byte = ((unsigned char *)&telemetry_data)[index+1];
   ...
   /* Start to send TM data registers starting from the first ones */
   {
      ...
      WRITE_TM_MSB (*((unsigned char*)&telemetry_data));
      telemetry_pointer++;
      WRITE_TM_MSB (*((unsigned char*)&telemetry_data + 1));
      telemetry_pointer++;  
   }
