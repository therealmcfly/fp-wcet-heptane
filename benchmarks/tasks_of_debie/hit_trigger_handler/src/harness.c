...
signed char WaitMail_execution_result = 0;
signed char WaitMail_wait_result = 0;
uint16_t WaitMail (incoming_mail_t EXTERNAL message) COMPACT_DATA REENTRANT_FUNC
...
/*                   in software_error register.                             */
{
   ...
   {
      WaitMail_execution_result = MSG_RECEIVED;
      WaitMail_wait_result = MSG_RECEIVED;
      ...
      return mail_message[message.mailbox_number];
   }
   else
   {
      WaitMail_execution_result = TIMEOUT_OCCURRED;
      WaitMail_wait_result = TIMEOUT_OCCURRED;
      //*(message.message)       = 0;
      return 0;
   }
...
{
   (NextIndex) ++;
   if ((NextIndex) >= AD_NUM_RAND) NextIndex = 0;
...
void Start_Conversion (void)
{
   ...
   NextIndex = ad_rand_index;
   ad_value = ((uint16_t)Next_Rand (ad_rand_index)) << 8;
   ad_rand_index = NextIndex;
   NextIndex = ad_rand_index;
   ...
   {
      NextIndex = ad_fail_index;
      if (Next_Rand (ad_fail_index) > 0x3f)
      {
          ad_fail_index = NextIndex;
 ...
 */
{
   ...
   {  /* Conversion still going on. */
      ...
      if (max_adc_hits > 0)
      {
         ...
         NextIndex = ad_hit_rand_index;
         if (Next_Rand (ad_hit_rand_index) > 0x7f)
         {
             ad_hit_rand_index = NextIndex;
            /* Hit me again, Sam. */
...
/* Sets random data in the event sensors. */
{
   ...
   NextIndex = event_rand_index;
   val = Next_Rand (event_rand_index);
   event_rand_index = NextIndex;
...
static unsigned char Send_Patch_Code (
   uint16_t       address,
...
static unsigned char Send_Patch_Data (
   uint16_t       address,
   unsigned char checksum)
...
/* Test of TelecommandExecutionTask. */
{
   ...
   CASE("TC to patch code memory, call patch");
      ...
      chsum = Send_Patch_Code (0x1100, chsum);
   ...
   CASE("TC to patch code memory, no action");
      ...
      chsum = Send_Patch_Code (0x1300, chsum);
   ...
   CASE("TC to patch code memory, soft reset");
      ...
      chsum = Send_Patch_Code (0x1400, chsum);
   ...
   CASE("TC to patch code memory, warm reset");
      ...
      chsum = Send_Patch_Code (0x2100, chsum);
   ...
   CASE("TC to patch code memory, invalid action");
      ...
      chsum = Send_Patch_Code (0x2400, chsum);
   ...
   CASE("TC to patch code memory, invalid address");
      ...
      chsum = Send_Patch_Code (0x0fff, chsum);
   ...
   CASE("TC to patch code memory, checksum error");
      ...
      chsum = Send_Patch_Code (0x1200, chsum);
   ...
   CASE("TC to patch data memory");
      ...
      chsum = Send_Patch_Data (0x2200, chsum);
   ...
   CASE("TC to patch data memory, address error");
      ...
      chsum = Send_Patch_Data (0xfef0, chsum);
   ...
   CASE("TC to patch data memory, checksum error");
      ...
      chsum = Send_Patch_Data (0x2300, chsum);
...
/* Tests of the Health Monitoring task. */
{
   ...
   CASE("Monitoring with SU errors, no interrupting hits");
      ...
      do
      {
#warning Extracted during a run of the bench.
          ANNOT_MAXITER(121);
...
#error Check the Check_Type_Size function in your target assembly. 
   ...
   CASE("Science Data TM");
      ...
      while (mail_count[TCTM_MAILBOX] == 0)
      {
          ...
          ANNOT_MAXITER(17676);
#else
#error sizeof(science_data_file_t) dependent bound.
#error Check the Check_Type_Size function in your target assembly. 
#endif
...
/* Tests of AcquisitionTask. */
{
   ...
   CASE("Hits with Science Data not full");
      ...
      {
#warning Depends on Random_Event, extracted during a run of the bench.
          ANNOT_MAXITER(2625);
         hits ++;
         internal_time ++;
         hit_budget_left = 10;
...
/* Tests of the "SU Self Test" function. */
{
   ...
   CASE("Run Self Test for SU2");
      ...
      {
          //Only 10 possible values for the health_mon_round 
          ANNOT_MAXITER(8);
         Monitor_Health (Prob6a);
      }
...
/* Executes the test scenario.                                               */
{
   ...
   while (TARGET_REPEAT_TEST && test_round < 50);
#warning Altered System Start to allow system termination.
