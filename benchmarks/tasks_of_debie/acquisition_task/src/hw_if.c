...

{
   ...
   {
       //From 0x1000 4096 to 0x8000 32768
       ANNOT_MAXITER(28672);
...
unsigned char extern memory_transfer_buffer[];
...

{
   ...
   {
       //According to all calls to this function in the bench, 32 should be a
       //better bound (MEM_BUFFER_SIZE). 255 is more generic.
       ANNOT_MAXITER(255);
      old_checksum ^= GET_DATA_BYTE(patch_variables.destination + i);
      patch_value   = memory_transfer_buffer[i];
...
unsigned short int ReadDelayCounters_FromPlasma1Minus = 0;
unsigned short int ReadDelayCounters (delays_t EXTERNAL delay)
...
/* Algorithm      :  MSB and LSB are combined to form an 16 bit int.         */
{
   ...
   ReadDelayCounters_FromPlasma1Minus = delay.FromPlasma1Minus;
   return delay.FromPlasma1Plus;
}
...
trigger_set_t SetTriggerLevel_result;
unsigned char SetTriggerLevel(trigger_set_t EXTERNAL setting) 
...

{
  ...
  SetTriggerLevel_result = setting;
  // SetTriggerLevel_base = setting.base;
   return setting.execution_result;
}
...
voltage_status_t GetVoltageStatus_result;
