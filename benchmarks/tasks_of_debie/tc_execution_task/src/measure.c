...

{
   ...
   WaitMail_execution_result = ACQ_mail.execution_result;
   WaitMail_wait_result = ACQ_mail.wait_result;
   trigger_unit = WaitMail(ACQ_mail);
   ACQ_mail.wait_result = WaitMail_wait_result;
   ACQ_mail.execution_result = WaitMail_execution_result;
...

   {
           ...
           && (EVENT_FLAG == ACCEPT_EVENT))
      {
         ...
         /*Event Record.                                                  */
         event_queue[event].SU_number = (unsigned char)(trigger_unit & 0xFF);

         /*Contents of a temporary buffer is stored into Event Record.    */
         COPY (event_queue[event]. plasma_1_plus ,ADC_result[0]);
         COPY (event_queue[event]. plasma_1_minus,ADC_result[1]);
         COPY (event_queue[event]. piezo_1       ,ADC_result[2]);    
         COPY (event_queue[event]. piezo_2       ,ADC_result[3]);
         COPY (event_queue[event]. plasma_2_plus ,ADC_result[4]);

         /*Rise time counter is read in to Event Record.                  */
         event_queue[event]. rise_time = ReadRiseTimeCounter();

         /*Delay counters are read in to a struct.                        */
         ReadDelayCounters_FromPlasma1Minus = delay_counters.FromPlasma1Minus;
         delay_counters.FromPlasma1Plus = ReadDelayCounters(delay_counters);  
         delay_counters.FromPlasma1Minus = ReadDelayCounters_FromPlasma1Minus;

         /*Delay from plasma 1+ to PZT 1/2 is stored into Event Record.   */
         COPY (event_queue[event]. delay_2,delay_counters.FromPlasma1Plus);                

         /*Delay from plasma 1- to PZT 1/2 is stored into Event Record.   */
         COPY (event_queue[event]. delay_3,delay_counters.FromPlasma1Minus);           
         ...
         {
            event_queue[event]. delay_1 = time_delay;
            /*Delay from plasma 1+ to plasma 1- is calculated and stored  */
            /*into Event Record.                                          */
         }

         /*Measurement time is stored into Event Record.                  */
         COPY (event_queue[event]. hit_time, internal_time);

         /*Unit temperatures are stored into Event Record.                */

         event_queue[event]. SU_temperature_1 = 
            telemetry_data.SU_temperature[trigger_unit - SU1][0];

         event_queue[event]. SU_temperature_2 = 
            telemetry_data.SU_temperature[trigger_unit - SU1][1];

         ClassifyEvent_Result_quality_number = event_queue[event].quality_number;
         event_queue[event].classification = ClassifyEvent(event_queue[event]);
         event_queue[event].quality_number = ClassifyEvent_Result_quality_number;
         /* New event is classified. */

         //checksum_pointer = ((EXTERNAL unsigned char *)(&(event_queue[event])))[0];
         event_checksum = 0;
...
#endif
            event_checksum ^= ((unsigned char*) &(event_queue[event]))[i-1];
            //event_checksum ^= *checksum_pointer;
            //checksum_pointer++;
         }

         //event_queue[event].checksum = event_checksum;
         {
             //asm("lui $2,0");
             //asm("lui $3,0");
             event_queue[event].checksum = event_checksum;
             //((unsigned char*) (event_queue))[(event * (((unsigned char*) event_queue + 1) - ((unsigned char *)event_queue)) + sizeof(event_record_t) - 1)] = event_checksum;
         }
...
sensor_unit_t Switch_SU_State_result;
...
unsigned char Start_SU_SwitchingOn(
      sensor_index_t SU,
...
unsigned char SetSensorUnitOff(
         sensor_index_t SU,
         unsigned char EXTERNAL exec_result) COMPACT_DATA REENTRANT_FUNC
