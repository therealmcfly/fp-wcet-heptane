/* MDH WCET BENCHMARK SUITE. File version $Id: lms.c,v 1.1.1.1 2007-12-11 15:22:56 puaut Exp $ */
...
float rad;
{
  ...
  while (rad > 2 * PI)
    {
      ANNOT_MAXITER(100);	// random, don't know
      rad -= 2 * PI;
    }
  while (rad < -2 * PI)
    {
  ...
  while (fabs(diff) >= 0.00001)
    {
      ANNOT_MAXITER(1000);	// random, don't know
      diff = (diff * (-(rad * rad))) / ((2.0 * inc) * (2.0 * inc + 1.0));
      app = app + diff;
      inc++;
    }
